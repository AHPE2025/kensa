...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  const { drawingId, issueId } = await params
  ...
  if (existingError || !existingIssue) {
    console.error('update issue position error:', existingError ?? 'issue not found')
  ...
  for (const key of ['pin_x', 'pin_y', 'callout_x', 'callout_y'] as const) {
    if (key in updates) {
      const ratio = toRatio(updates[key])
      if (ratio === undefined) {
...
export async function DELETE(request: NextRequest, { params }: Params) {
  ...
  if (existingError || !existingIssue) {
    console.error('delete issue error:', existingError ?? 'issue not found')
    return NextResponse.json({ error: '指摘が見つかりません' }, { status: 404 })
  }
